...
int main(int argc, char*argv[])
{
    ...
    /* TODO on the diagram we used had a=5, b=2 and c=6 */
    /* We can also use things like M_SQRT2 */
    ...
    /* TODO here we hack in a target plane for the reflected
     *      ray.
     *
     * where we try to hack in a plane at y=4 with a normal 
     * that is -j_hat.  */
     cplex_vec_set( &plane_point, 0.0, 0.0, 4.0, 0.0, 0.0, 0.0);
    ...
    while(1){
        ...
        if ( button == Button1 ){
            ...
            if (    ( mouse_x >=  offset_x ) && ( mouse_y >= offset_y )
                ...
                printf("L_0 = < %-+18.12e, %-+18.12e, %-+18.12e >\n",
                        obs_point.x.r, obs_point.y.r, obs_point.z.r );

                sprintf(buf,"L_0 = < %-+12.6e, %-+12.6e, %-+12.6e >",
                        obs_point.x.r, obs_point.y.r, obs_point.z.r );
                ...
                /* TODO determine if any of these signed zeros
                 * matter. */
                if ( k_val[0].r == 0.0 ) k_val[0].r = 0.0;
                if ( k_val[0].i == 0.0 ) k_val[0].i = 0.0;
                if ( k_val[1].r == 0.0 ) k_val[1].r = 0.0;
                if ( k_val[1].i == 0.0 ) k_val[1].i = 0.0;
                ...
                if ( intercept_cnt > 0 ) {
                    ...
                    if ( surface_icept_pt( &hit_point,
                        ...
                        /*TODO don't do this long term. Use the 
                        ...
                        sprintf(buf, "H = ( %-+12.6e, %-+12.6e, %-+12.6e )",
                                                   hit_point.x.r,
                                                   hit_point.y.r,
                                                   hit_point.z.r );
                        ...
                        sprintf(buf, "N = < %-+12.6e, %-+12.6e, %-+12.6e >",
                                                   grad_norm.x.r,
                                                   grad_norm.y.r,
                                                   grad_norm.z.r );
                        ...
                        if ( ( vec_T_mag < RT_EPSILON )
                             ...
                             * plane interception. 
                             *
                             * On that note we have certain assumptions : 
                             *
                             *      theta_i is zero or near zero 
                             *
                             *      Rr reflection is the same as the surface gradiant
                             */
                            ...
                            /* stop looking at this over and over as the darned
                            ...
                            if ( cplex_cramer( tmp+7, tmp+3, tmp+4, tmp+5, tmp+6 ) == MATH_OP_FAIL ) {
                                 ...
                                 * computed surface intercept point H is hit_point.
                                 */
                                ...
                                if ( lp_status != 0 ) {
                                    ...
                                    printf("     : line plane intercept = no intercept\n");
                                }
                            }
                        }
                    }
                    ...
                    /* TODO determine if this code will ever happen */
                    ...
                    sprintf(buf,"root 0 = ------ no solution ------",
                        k_val[0].r, k_val[0].i );

                    XDrawImageString( dsp, win3, gc3, 10, 110,
                                  buf, strlen(buf));

                    sprintf(buf,"root 1 = ------ no solution ------",
                        k_val[1].r, k_val[1].i );
                ...
                /* if we have any negative signed zeros then perhaps
                 * we can scrub them.
                 *
                 * TODO is this necessary again ?? */
