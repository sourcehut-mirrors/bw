...
/* sha.h was modified by Dennis Clarke to remove 32-bit code chunks */
#include "sha512.h"
...
/* strangely there never was a prototype for this */
...
static struct hmachash {
    ...
    size_t resultlength[5];
} hmachashes[HMACTESTCOUNT] = {
  { /* 1 twenty binary zero bits */ {
...
int checkmatch(const unsigned char *hashvalue, const char *hexstr, size_t hashsize) {
  int i;
  for (i = 0; (size_t)i < hashsize; ++i) {
 ...
 */
void printxstr(const char *str, size_t len) {
  for ( ; len-- > 0; str++)
 ...
 */
void printResult(uint8_t *Message_Digest, size_t hashsize,
    ...
    const char *resultarray, int printResults, int printPassFail)
{
  ...
  if (printResults == PRINTTEXT) {
    ...
    putchar('\n');
      ...
      /* note that fwrite uses size_t args thus 
       *
       ...
       *
       */
    fwrite(Message_Digest, (size_t)1, hashsize, stdout);
  ...
  if (printResults && resultarray) {
    printf("    Should match:\n\t");
    for (i = 0, k = 0; (size_t)i < hashsize; i++, k += 2) {
 ...
 */
int hash(int testno, int loopno, SHAversion hashno,
  const char *testarray, size_t length, long repeatcount,
  int numberExtrabits, int extrabits, const unsigned char *keyarray,
  size_t keylen, const char *resultarray, size_t hashsize, int printResults,
  int printPassFail)
{
  USHAContext sha;
  HMACContext hmac;
  ...
  if (printResults == PRINTTEXT) {
    ...
    printf("'\n");
    printf("    Length=%lu bytes (%lu bits), ", length, length * 8);
  ...
  for (i = 0; i < repeatcount; ++i) {
    ...
    err = keyarray ? hmacInput(&hmac, (const uint8_t *) testarray, length) :
                     USHAInput(&sha, (const uint8_t *) testarray, (unsigned int)length);
  ...
  if (numberExtrabits > 0) {
    err = keyarray ? hmacFinalBits(&hmac, (uint8_t) extrabits, (unsigned int)numberExtrabits) :
                     USHAFinalBits(&sha, (uint8_t) extrabits, (unsigned int)numberExtrabits);
    if (err != shaSuccess) {
 ...
 */
int hashfile(SHAversion hashno, const char *hashfilename, uint8_t bits,
  int bitcount, int skipSpaces, const unsigned char *keyarray,
  int keylen, const char *resultarray, size_t hashsize,
  int printResults, int printPassFail)
{
  ...
  if (skipSpaces)
    while ((c = getc(hashfp)) != EOF) {
      if (!isspace(c)) {
        ...
        err = keyarray ? hmacInput(&hmac, &cc, (size_t)1) :
                         USHAInput(&sha, &cc, (unsigned int)1);
        ...
        while ((nread = fread(buf, 1, sizeof(buf), hashfp)) > 0) {
            err = keyarray ? hmacInput(&hmac, buf, nread) : USHAInput(&sha, buf, (unsigned int)nread);
            if (err != shaSuccess) {
  ...
  if (bitcount > 0)
    err = keyarray ? hmacFinalBits(&hmac, bits, (unsigned int)bitcount) : USHAFinalBits(&sha, bits, (unsigned int)bitcount);
  if (err != shaSuccess) {
    fprintf(stderr, "hashfile(): %s Error %d.\n",
 ...
 */
void randomtest(SHAversion hashno, const char *seed,
                size_t hashsize, const char **resultarrays,
                int randomcount, int printResults,
                int printPassFail) {
 ...
 * Look up a hash name.
 */
SHAversion findhash(const char *argv0, const char *opt)
{
  int i;
  ...
  /* the usage() function was modified to return EXIT_FAILURE */
  exit (usage(argv0));
 ...
 */
void testErrors(SHAversion hashnolow, SHAversion hashnohigh, int printResults, int printPassFail) {
  USHAContext usha;
  uint8_t Message_Digest[USHAMaxHashSize];
  int err;
  SHAversion hashno;
  ...
  for (hashno = hashnolow; hashno <= hashnohigh; hashno++) {
    ...
    err = USHAInput(&usha, (const unsigned char *)"foo", (unsigned int)3);
    if (printResults == PRINTTEXT)
      printf ("\nError %d. Should be %d.\n", err, shaStateError);
...
int main(int argc, char **argv)
{
  ...
  SHAversion hashno;
  SHAversion hashnolow = 0;
  SHAversion hashnohigh = HASHCOUNT - 1;
  ...
  while ((i = xgetopt(argc, argv, "b:B:ef:F:h:Hk:l:mpPr:R:s:S:t:wxX"))
         != -1)
    switch (i) {
      case 'b': /* this is weak as hell as no effort was made to
                 * check that the input can be a uint8_t */
                extrabits = (uint8_t)strtol(xoptarg, 0, 0);
                break;
      ...
      case 'h': /* these must be enum SHAversion which can only be
                   SHA1, SHA224, SHA256, SHA384, SHA512 as the 
                   numbers 0 to 4 */
      ...
      case 's': hashstr = xoptarg; hashlen = strlen(hashstr);
                break;
      ...
      default: /* usage() function modified to return EXIT_FAILURE */
                return usage(argv[0]);
      }
  ...
  /* Note that the testing of SHAversion data being less than zero
   * is pointless. See the findhash() function for details. */
  if ((testnolow < 0) ||
      ...
      /* display the usage() and that function return EXIT_FAILURE */
      return usage(argv[0]);
  }
