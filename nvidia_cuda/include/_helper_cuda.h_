...
inline int _ConvertSMVer2Cores(int major, int minor)
{
    ...
    fprintf(stderr,"WARN : MapSMtoCores for SM %d.%d is undefined.\n", major, minor);
    fprintf(stderr,"     : Default to use %d Cores/SM\n", nGpuArchCoresPerSM[index-1].Cores);
...
inline int gpuGetMaxGflopsDeviceId()
{
    ...
    if (device_count == 0) {
        fprintf(stderr, "FAIL : gpuGetMaxGflopsDeviceId() CUDA error: no devices supporting CUDA.\n");
        exit(EXIT_FAILURE);
    }
    ...
    while (current_device < device_count) {
        ...
        /* only if this GPU is not running on Compute Mode prohibited */
        if (deviceProp.computeMode != cudaComputeModeProhibited) {
    ...
    if (devices_prohibited == device_count) {
    	fprintf(stderr, "FAIL : gpuGetMaxGflopsDeviceId() CUDA error: all devices have compute mode prohibited.\n");
    	exit(EXIT_FAILURE);
    }
