...
int main ( int argc, char *argv[] ) {
     ...
     *   seee eeee eeee dddd dddd ---> the rest is data where s is sign bit and e is exponent
     *
     *   
     * 
     * 
     * 
     *   s100 0000 0000 data begins ^C
     * 
     * 64 bit uint   80 00 00 00 00 00 00 01  <-- a 1 bit on each end ^C
     * 
     * uint64_t      80 01 02 03 04 05 06 07  <-- a 1 bit on each end and numbered 0 to 7 ^C
     * 
    ...
    printf("\n");
    /* do not even bother to try to print this baloney out */
    printf("-------------------------------------------------\n");

    uint8_t pi_fp64le_data[16] = { 0x18, 0x2d, 0x44, 0x54, 0xfb, 0x21, 0x09, 0x40,
                                   0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00 };

    uint8_t *pi_fp64le = calloc((size_t)16,sizeof(uint8_t));
    for ( j = 0; j < 16; j++ ) {
        *(pi_fp64le + j) = pi_fp64le_data[j];
    }
