...
C     ******************************************************************
  ...
  100 CONTINUE
      ...
      WGAME(12)=4
C     WGAME(13)=O'113'
C     FOR SOME OBSCURE REASON THE 13TH ELEMENT WAS A LETTER O'113'
C       PERHAPS THE OCTAL 0113 WILL WORK?
      WGAME(13)=O'113'
      WGAME(15)=5
      WGAME(16)=1
      WGAME(17)=4
...
C     ******************************************************************
  ...
  100 CONTINUE
      ...
      CALL TELER(INBUF,7)
C
C     LETS UNLEASH THE AWESOME POWER OF INBUF
C
      WRITE (0,*) 'IN READB, TELER RETURNED >', INBUF, '<'
      WRITE (0,9999) (INBUF(I:I),I=1,8)
C
C     REMEMBER TO COMMENT THAT CRAP
C
 9999 FORMAT (1X,8Z3.2)
C
C     WE REALLY NEED TO COMMENT OUT THAT STUFF
C
C     IF(INBUF.NE.ICHAR('M')) GO TO 110
      IF(INBUF.NE.O'115') GO TO 110
      CALL LEGAL
      GO TO 100
  110 IF(INBUF.NE.O'120') GO TO 120
      CALL PRINT
      GO TO 100
  ...
  130 IF(INBUF(1:1).NE.'-') GO TO 200
      ...
      GOTO 02120
  200 IF(INBUF.NE.O'122') GO TO 300
      CALL WINDUP
  300 IF(INBUF.NE.O'113') GO TO 400
      CALL BKINGOO(*1000)
      RETURN 1
  400 IF(INBUF.NE.O'121') GO TO 500
      CALL BQUEENOO(*1000)
      RETURN 1
  500 IF(LEN_TRIM(INBUF).LT.4) GO TO 1000
...
C     FIND AVAILABLE SLOT
  ...
  100 CONTINUE
      RETURN 1
C     DON'T CREATE A PIECE ON TOP OF ANOTHER
  110 IF(BOARD(X,Y) .NE. 0) RETURN 1
C     AOK - CREATE IT
      SQUARES(2,2,I)=P
      SQUARES(1,1,I)=X
      SQUARES(1,2,I)=Y
...
C     ******************************************************************
  ...
  100 CALL TELEW(MESS1,25)
      CALL TELER(INBUF,2)
      IF(INBUF.EQ.O'121') PIECE=5
      IF(INBUF.EQ.O'122') PIECE=4
      IF(INBUF.EQ.O'102') PIECE=3
      IF(INBUF.EQ.O'116') PIECE=2
...
C     ******************************************************************
     ...
     X           (BSQUARES(1,1,1),I2(1089)),(CONTROL(1,1,-16),I2(2113))
      ...
      T4=WGAME(INDEX+3)
C
C     WE HAD THE LETTER O'113' SOMEHOW SHOVED INTO WGAME(13) 
C     HOWEVER WE CAN NOT COMPARE AN INTEGER TO A LETTER
C     THEREFORE WE GO WITH THE OCTAL 0113 FOR ASCII O'113'
C     IF(T1.NE.O'113') GO TO 50
      IF(T1.NE.O'113') GO TO 50
      CALL WKINGOO(*300)
      MNEXT=1
      GO TO 140
   50 IF(T1.NE.O'121') GO TO 60
      CALL WQUEENOO(*300)
      MNEXT=2
  140 WKCASTLE=.FALSE.
      WQCASTLE=.FALSE.
...
C     ******************************************************************
     ...
     X           (BSQUARES(1,1,1),I2(1089)),(CONTROL(1,1,-16),I2(2113))
      ...
      DATA REP/O'120',O'116',O'102',O'122',O'121',O'113'/
      COUNT=0
      INDEX=(MOVENUM*4)-3
      DO 200 I=1,16
...
C     WRITE (0,*) 'IN TELER, ISIZE = ', ISIZE, ', BUT LL = ', LL
  ...
  100 FORMAT (A)
      L = LEN_TRIM(STRING)
      WRITE (0,*) 'GOT STRING >', STRING, '< WITH LEN_TRIM = ', L
      DO 110 I=1,L
